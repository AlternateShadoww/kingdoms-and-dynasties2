...
public class GihakamaRenderer extends DyeableGeoArmorRenderer<GihakamaItem> {
    ...
    protected @NotNull Color getColorForBone(GeoBone geoBone) {
        ...
        if (itemStack.getItem() instanceof GihakamaItem armorItem) {
            ...
            if (boneName.endsWith(GihakamaRenderer.PRIMARY_PART)) {
                return applyDyeToBone(geoBone, primaryColor, GihakamaRenderer.PRIMARY_PART);
            } else if (boneName.endsWith(GihakamaRenderer.SECONDARY_PART)) {
                return applyDyeToBone(geoBone, secondaryColor, GihakamaRenderer.SECONDARY_PART);
            }
        }
