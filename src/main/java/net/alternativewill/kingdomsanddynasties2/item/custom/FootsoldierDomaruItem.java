...
import net.alternativewill.kingdomsanddynasties2.item.client.FootsoldierDomaruRenderer;
...
public class FootsoldierDomaruItem extends DyeableArmorItem implements GeoItem {
    ...
    public FootsoldierDomaruItem(ArmorMaterial pMaterial, Type pType, Properties pProperties) {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private FootsoldierDomaruRenderer renderer;

            @Override
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack,
                                                                   EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new FootsoldierDomaruRenderer();
            ...
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                return RendererInstances.FOOTSOLDIERDOMARU_ITEM_RENDERER;
            }
        });
    }
    ...
    public void setPrimaryColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, FootsoldierDomaruItem.PRIMARY_PART, color, STANDARD_PRIMARY_COLOR);
    }

    public void setSecondaryColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, FootsoldierDomaruItem.SECONDARY_PART, color, STANDARD_SECONDARY_COLOR);
    }

    public void setGoldColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, FootsoldierDomaruItem.GOLD_PART, color, STANDARD_GOLD_COLOR);
    }

    public void setSilverColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, FootsoldierDomaruItem.SILVER_PART, color, STANDARD_SILVER_COLOR);
    }

    public int getPrimaryColor(@NotNull ItemStack stack) {
        return getColorTag(stack, FootsoldierDomaruItem.PRIMARY_PART, STANDARD_PRIMARY_COLOR);
    }

    public int getSecondaryColor(@NotNull ItemStack stack) {
        return getColorTag(stack, FootsoldierDomaruItem.SECONDARY_PART, STANDARD_SECONDARY_COLOR);
    }

    public int getGoldColor(@NotNull ItemStack stack) {
        return getColorTag(stack, FootsoldierDomaruItem.GOLD_PART, STANDARD_GOLD_COLOR);
    }

    public int getSilverColor(@NotNull ItemStack stack) {
        return getColorTag(stack, FootsoldierDomaruItem.SILVER_PART, STANDARD_SILVER_COLOR);
    }

    @Override
    public int getColor(@NotNull ItemStack stack) {
        return getColorTag(stack, FootsoldierDomaruItem.CRAFTING_TABLE_PART, STANDARD_CRAFTING_TABLE_COLOR);
    }
    ...
    public void undoColor(ItemStack stack, int buttonIndex) {
        if (stack.getItem() instanceof FootsoldierDomaruItem yoroiArmorItem) {
            switch (buttonIndex) {
