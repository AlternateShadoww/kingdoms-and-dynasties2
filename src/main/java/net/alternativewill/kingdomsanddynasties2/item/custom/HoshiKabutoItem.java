...
import net.alternativewill.kingdomsanddynasties2.item.client.HoshiKabutoRenderer;
...
public class HoshiKabutoItem extends DyeableArmorItem implements GeoItem {
    ...
    public HoshiKabutoItem(ArmorMaterial pMaterial, Type pType, Properties pProperties) {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private HoshiKabutoRenderer renderer;

            @Override
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack,
                                                                   EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new HoshiKabutoRenderer();
            ...
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                return RendererInstances.HOSHI_ITEM_RENDERER;
            }
        });
    }
    ...
    public void setPrimaryColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, HoshiKabutoItem.PRIMARY_PART, color, STANDARD_PRIMARY_COLOR);
    }

    public void setSecondaryColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, HoshiKabutoItem.SECONDARY_PART, color, STANDARD_SECONDARY_COLOR);
    }

    public void setGoldColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, HoshiKabutoItem.GOLD_PART, color, STANDARD_GOLD_COLOR);
    }

    public void setSilverColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, HoshiKabutoItem.SILVER_PART, color, STANDARD_SILVER_COLOR);
    }

    public int getPrimaryColor(@NotNull ItemStack stack) {
        return getColorTag(stack, HoshiKabutoItem.PRIMARY_PART, STANDARD_PRIMARY_COLOR);
    }

    public int getSecondaryColor(@NotNull ItemStack stack) {
        return getColorTag(stack, HoshiKabutoItem.SECONDARY_PART, STANDARD_SECONDARY_COLOR);
    }

    public int getGoldColor(@NotNull ItemStack stack) {
        return getColorTag(stack, HoshiKabutoItem.GOLD_PART, STANDARD_GOLD_COLOR);
    }

    public int getSilverColor(@NotNull ItemStack stack) {
        return getColorTag(stack, HoshiKabutoItem.SILVER_PART, STANDARD_SILVER_COLOR);
    }

    @Override
    public int getColor(@NotNull ItemStack stack) {
        return getColorTag(stack, HoshiKabutoItem.CRAFTING_TABLE_PART, STANDARD_CRAFTING_TABLE_COLOR);
    }
    ...
    public void undoColor(ItemStack stack, int buttonIndex) {
        if (stack.getItem() instanceof HoshiKabutoItem HoshiKabutoItem) {
            switch (buttonIndex) {
                case 0: // Primary color
                    HoshiKabutoItem.undoColorTag(stack, PRIMARY_PART, STANDARD_PRIMARY_COLOR);
                    break;
                case 1: // Secondary color
                    HoshiKabutoItem.undoColorTag(stack, SECONDARY_PART, STANDARD_SECONDARY_COLOR);
                    break;
                case 2: // Gold part
                    HoshiKabutoItem.undoColorTag(stack, GOLD_PART, STANDARD_GOLD_COLOR);
                    break;
                case 3: // Silver part
                    HoshiKabutoItem.undoColorTag(stack, SILVER_PART, STANDARD_SILVER_COLOR);
                    break;
                default:
                    break;
            }
