...
import net.alternativewill.kingdomsanddynasties2.item.client.WarajiRenderer;
...
public class WarajiItem extends DyeableArmorItem implements GeoItem {
    private final AnimatableInstanceCache cache = new SingletonAnimatableInstanceCache(this);

    // Standard Colors
    public static final int STANDARD_PRIMARY_COLOR = 1843249;
    public static final int STANDARD_SECONDARY_COLOR = 14401916;
    ...
    public WarajiItem(ArmorMaterial pMaterial, Type pType, Properties pProperties) {
    ...
    public void initializeClient(Consumer<IClientItemExtensions> consumer) {
        consumer.accept(new IClientItemExtensions() {
            private WarajiRenderer renderer;

            @Override
            public @NotNull HumanoidModel<?> getHumanoidArmorModel(LivingEntity livingEntity, ItemStack itemStack,
                                                                   EquipmentSlot equipmentSlot, HumanoidModel<?> original) {
                if (this.renderer == null)
                    this.renderer = new WarajiRenderer();
            ...
            public BlockEntityWithoutLevelRenderer getCustomRenderer() {
                return RendererInstances.WARAJI_ITEM_RENDERER;
            }
        });
    }
    ...
    public void setPrimaryColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, WarajiItem.PRIMARY_PART, color, STANDARD_PRIMARY_COLOR);
    }

    public void setSecondaryColor(@NotNull ItemStack stack, int color) {
        setColorTag(stack, WarajiItem.SECONDARY_PART, color, STANDARD_SECONDARY_COLOR);
    }

    public int getPrimaryColor(@NotNull ItemStack stack) {
        return getColorTag(stack, WarajiItem.PRIMARY_PART, STANDARD_PRIMARY_COLOR);
    }

    public int getSecondaryColor(@NotNull ItemStack stack) {
        return getColorTag(stack, WarajiItem.SECONDARY_PART, STANDARD_SECONDARY_COLOR);
    }
    ...
    public void undoColor(ItemStack stack, int buttonIndex) {
        if (stack.getItem() instanceof WarajiItem WarajiItem) {
            switch (buttonIndex) {
                case 0: // Primary color
                    WarajiItem.undoColorTag(stack, PRIMARY_PART, STANDARD_PRIMARY_COLOR);
                    break;
                case 1: // Secondary color
                    WarajiItem.undoColorTag(stack, SECONDARY_PART, STANDARD_SECONDARY_COLOR);
                    break;
                default:
                    break;
            }
